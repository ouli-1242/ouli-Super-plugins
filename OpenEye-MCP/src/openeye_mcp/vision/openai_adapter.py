"""OpenAI 兼容视觉适配器。

通过 OpenAI Chat Completions API（兼容任何 OpenAI 格式服务，例如 Azure
OpenAI、代理、第三方兼容服务）发送图文请求并返回文本。
"""

from __future__ import annotations

import httpx

from openeye_mcp.config import settings
from openeye_mcp.vision._http import get_client as _get_client
from openeye_mcp.vision._retry import post_with_retry
from openeye_mcp.vision.base import VisionAdapter, VisionOptions

_DEFAULT_BASE_URL = "https://api.openai.com/v1"

# 推理模型（如 step_plan 端点）偶发 content 为空：重试次数
_EMPTY_CONTENT_RETRIES = 1


def _extract_message(data: dict) -> dict | None:
    """从 Chat Completions 响应中稳健提取首个 message。

    部分后端偶发返回病态响应（``choices`` 缺失 / 为 None / 空数组），
    结构异常时返回 None，由调用方按「空内容」降级处理。
    """
    try:
        message = data["choices"][0]["message"]
    except (KeyError, IndexError, TypeError):
        return None
    return message if isinstance(message, dict) else None


def _message_text(message: dict | None, *, allow_reasoning: bool = False) -> str:
    """取 message 的正文；正文为空且允许时可回退到 reasoning_content。"""
    if not message:
        return ""
    content = (message.get("content") or "").strip()
    if content or not allow_reasoning:
        return content
    return (message.get("reasoning_content") or "").strip()


class OpenAIVisionAdapter(VisionAdapter):
    """基于 OpenAI Chat Completions 的视觉适配器。

    Args:
        model: 模型名称，未指定时使用 ``settings.openai_model``。
        api_key: API Key，未指定时使用 ``settings.openai_api_key``。
        base_url: 接口地址，未指定时使用 ``settings.openai_base_url``，
            仍为空则回退到官方 ``https://api.openai.com/v1``。
    """

    settings_prefix = "openai"
    default_base_url = _DEFAULT_BASE_URL

    @property
    def _url(self) -> str:
        return f"{self.base_url.rstrip('/')}/chat/completions"

    def _base_payload(self, options: VisionOptions) -> dict:
        """模型名与生成参数部分；messages 由调用方按带图/纯文本补。"""
        payload: dict = {"model": self.model, "max_tokens": options.max_tokens}
        # reasoning_effort 仅在显式配置时发送：部分后端不支持该参数
        if options.reasoning_effort:
            payload["reasoning_effort"] = options.reasoning_effort
        if options.response_format:
            payload["response_format"] = options.response_format
        return payload

    async def describe(
        self,
        image_b64: str,
        mime_type: str,
        prompt: str,
        max_tokens: int | None = None,
        reasoning_effort: str | None = None,
        response_format: dict | None = None,
    ) -> str:
        """调用 OpenAI Chat Completions 返回图像理解文本。

        Raises:
            httpx.HTTPStatusError: API 返回非 2xx 状态码时由
                ``raise_for_status()`` 抛出。
            RuntimeError: 超时或网络传输失败时带中文建议地抛出。
        """
        payload = self._base_payload(
            VisionOptions.resolve(max_tokens, reasoning_effort, response_format)
        )
        payload["messages"] = [
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": prompt},
                    {
                        "type": "image_url",
                        "image_url": {"url": f"data:{mime_type};base64,{image_b64}"},
                    },
                ],
            }
        ]
        headers = self.bearer_headers()
        client = _get_client()
        timeout = settings.request_timeout
        # 重试交给共享助手；这里只把「已耗尽」的失败翻译成带建议的中文文案
        try:
            response = await post_with_retry(client, self._url, json=payload, headers=headers)
        except httpx.TimeoutException as exc:
            raise RuntimeError(
                f"视觉模型请求超时（{timeout}s），已重试 {settings.max_retries} 次。"
                f"建议：1) 缩短 prompt；2) 增大 REQUEST_TIMEOUT；3) 换更快的 API 端点。"
            ) from exc
        except httpx.TransportError as exc:
            raise RuntimeError(
                f"网络传输错误：{exc}。建议检查网络或 OPENAI_BASE_URL 配置。"
            ) from exc

        message = _extract_message(response.json())
        content = _message_text(message)
        if content:
            return content

        # content 为空：推理模型偶发把答案放到 reasoning_content，先重试拿到干净答案
        for _ in range(_EMPTY_CONTENT_RETRIES):
            try:
                response = await post_with_retry(
                    client, self._url, json=payload, headers=headers
                )
            except (httpx.TimeoutException, httpx.TransportError):
                break
            message = _extract_message(response.json())
            content = _message_text(message)
            if content:
                return content

        # 重试仍空，回退到 reasoning_content（含完整答案，但可能带思考过程）
        return _message_text(message, allow_reasoning=True)

    async def describe_text(self, prompt: str) -> str:
        """纯文本请求：messages 只有 user 文本，无图片。"""
        payload = self._base_payload(VisionOptions.resolve())
        payload["messages"] = [{"role": "user", "content": prompt}]
        response = await post_with_retry(
            _get_client(), self._url, json=payload, headers=self.bearer_headers()
        )
        return _message_text(_extract_message(response.json()), allow_reasoning=True)
