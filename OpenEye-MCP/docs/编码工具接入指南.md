...
本文档详细说明如何将 OpenEye 接入各类主流 coding agent / MCP 客户端。安装、配置项与工具语义见 [README](../README.md)，本文只讲各客户端的接入差异。

## 简介

OpenEye 是一个 **stdio 类型的 MCP Server**，为纯文本模型（如 DeepSeek V4 Flash）提供视觉能力。它通过标准输入输出以 JSON-RPC 协议与 MCP 客户端通信，将图像理解、视觉推理等能力以 MCP 工具的形式暴露给宿主应用。OpenEye 提供六个核心工具：`describe_image`（图像描述）、`extract_text`（OCR 文字提取）、`ask_about_image`（视觉问答）、`analyze_layout`（UI 布局结构化分析，返回布局类型与元素树 JSON，适合前端复刻）、`extract_table`（表格转 Markdown，合并单元格附带 JSON）、`analyze_images`（多张图批量分析 + 跨图汇总）。

启动命令：`openeye`（`pip install openeye-mcp` 注册的 console script 入口），等价于 `python -m openeye_mcp.server`。配置通过环境变量传递（pydantic-settings 读取，环境变量优先于 `.env` 文件）。

**为什么要接入 coding agent**：当前主流的 coding agent / AI 编辑器（Claude Code、Cursor、Cline、Windsurf 等）均已原生支持 MCP。把 OpenEye 接入这些工具后，原本只能处理文本的 coding agent 就能在编码流程中调用视觉能力——例如分析 UI 截图、读取图表、理解设计稿、识别报错截图等，从而显著扩展纯文本模型在开发场景下的适用边界。由于 OpenEye 是 stdio 类型 server，各客户端的接入方式高度相似但配置文件位置、字段命名、JSON 结构存在差异，本文档对这些差异逐一梳理。

### OpenEye 关键信息速查

| 项 | 值 |
|---|---|
| 启动命令 | `openeye`（等价 `python -m openeye_mcp.server`） |
| 传输方式 | stdio（标准输入输出 JSON-RPC） |
| 配置方式 | 环境变量（优先于 `.env` 文件） |
| Windows 启动命令 | `<你的 Python 安装目录>/python.exe -m openeye_mcp.server`（`python` 不在 PATH 时用完整路径） |
| macOS/Linux 启动命令 | `python -m openeye_mcp.server` |
...
| `OCR_BACKEND` | `extract_text` 单独用哪个后端 | 空 = 跟随 `VISION_PROVIDER` |
| `GEMINI_BASE_URL` | Gemini 端点（自建代理 / 兼容网关时填） | Google 官方 |
| `REASONING_EFFORT` | 推理深度 `low`/`medium`/`high`（留空不发送） | 空 |
| `RETRY_BACKOFF` | 重试退避基数（秒，指数 + 抖动；429/5xx 优先按 `Retry-After`） | `0.5` |
| `REQUEST_DEADLINE` | **单次工具调用总预算**（秒，覆盖全部重试与降级；`0` 不设限） | `300` |
| `IMAGE_CACHE_TTL` / `IMAGE_CACHE_MAX_SIZE` | 已下载图片的缓存时长 / 条数（多轮重试不再重复下载解码） | `300` / `8` |
| `MAX_IMAGE_BYTES` | 图片来源大小上限（字节） | `20971520` |
| `ALLOW_PRIVATE_URLS` | 允许 URL 指向内网/保留地址（仅本地调试，会削弱 SSRF 防护） | `false` |
| `ALLOWED_IMAGE_ROOTS` | 逗号分隔本地目录白名单；非空则白名单外一律拒绝 | 空 |
| `LOG_LEVEL` | stderr 日志级别 | `INFO` |

> 本文示例统一以 OpenAI 后端为例。启动命令统一用 `openeye`（`pip install` 后自动注册到 PATH）；若不在 PATH，可用 `python -m openeye_mcp.server` 或完整路径。
...
执行 `opencode mcp list`，OpenEye 状态应为 `connected`。
...
- `codex mcp get openeye`：查看 OpenEye 详细配置与连接状态。
...
执行 `claude mcp list`，OpenEye 状态应为 `✔ Connected`。
...
- 在对话中让 Cline 调用 OpenEye 工具，未在 `autoApprove` 中列出的工具每次调用前会请求确认。
...
- 打开 Settings → Tools & MCP 面板，查看 OpenEye 的状态指示（绿色 = 正常，红色 = 连接失败）。
- Cursor 支持配置热重载，修改 `mcp.json` 后无需重启即可生效，在面板中确认状态。
- 在 Chat / Agent 模式下让 Cursor 调用 OpenEye 工具验证。
...
- 测试：在 Cascade 中提示 "使用 OpenEye 分析这张截图"。
...
- 打开 Settings → AI → MCP Servers，查看 OpenEye 名称旁的指示点：绿色 = "Server is active"，其他颜色查看 tooltip 提示。
- 打开 Agent Panel（`Cmd/Ctrl+Shift+A`）对话，在提示中提及 "openeye" 以帮助模型路由到对应工具。
...
- 在 Roo Code MCP 设置视图查看 OpenEye 的状态与已发现工具列表。
- 在对话中让 Roo 调用 OpenEye 工具，未在 `alwaysAllow` 中的工具每次调用前会请求确认。
...
OpenEye 使用 pydantic-settings 读取配置，**环境变量优先级高于 `.env` 文件**。各 coding agent 都通过 MCP 配置中的 `env` / `environment` 字段把变量注入到 OpenEye 子进程的环境变量中，这等价于直接设置环境变量，会覆盖项目 `.env` 文件中的同名值。因此推荐在 MCP 配置的 env 字段中直接填写所需的视觉后端凭证，而不依赖 `.env`。
...
- 也可直接用 `"command": "python"` + `"args": ["-m", "openeye_mcp.server"]`，此时依赖 PATH 中的 `python`。
- 在 TOML 配置文件（Codex CLI）中，字符串内反斜杠无需双写，但建议 Windows 路径直接用正斜杠 `/` 以避免歧义。

### 3. 启动命令与绝对路径

- OpenEye 通过 PyPI 分发（`pip install openeye-mcp`，见 README「安装」），启动命令统一为 `openeye`（等价 `python -m openeye_mcp.server`）。
- 多数 coding agent（尤其从 GUI 启动的 VS Code 类插件）不一定继承终端的 shell PATH，**强烈建议在 `command` 中使用绝对路径**而非依赖 `openeye` 命令的 PATH 解析，可避免 "command not found" 类问题。
- 推荐形式：`command` 为 Python 绝对路径，`args` 为 `["-m", "openeye_mcp.server"]`。Windows 示例：`"command": "C:/<你的 Python 安装目录>/python.exe", "args": ["-m", "openeye_mcp.server"]`。
...
OpenEye 是 **stdio 类型** MCP Server，配置时只需 `command`（+ `args`）与 `env`，**不要**填写 `url` / `serverUrl` / `headers` 等 HTTP 相关字段。多数客户端中，stdio 与 HTTP 传输互斥，同一 server 不能同时配置命令与 URL。

### 7. 配置生效与验证通用流程

1. 写入对应客户端的配置文件（或用 CLI/UI 添加）。
2. 按需重启客户端或重新加载配置（Cursor 支持热重载；Windsurf、Zed 等通常需要重启）。
3. 在客户端的 MCP 管理面板查看 OpenEye 连接状态（绿色 / connected / active 即正常）。
4. 在对话中显式让 agent 调用 OpenEye 的视觉工具验证端到端可用性。
5. 排查问题时优先在终端手动运行启动命令（`python -m openeye_mcp.server`）确认进程能正常拉起、环境变量无误。
