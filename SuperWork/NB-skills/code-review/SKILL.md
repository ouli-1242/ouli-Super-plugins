...
- Do NOT use: deciding how to integrate finished work (finishing-a-development-branch).

## Capability Boundary

- CAN: pin the diff base, locate spec and standards sources, run both axes as parallel sub-agents (or sequentially, kept separate), aggregate, persist the report.
- CANNOT: merge or re-rank findings across axes; force implementation of findings.
...
- Optional: a spec path; issue-tracker workflow (`docs/agents/issue-tracker.md`).
- Missing behavior: no fixed point given → ask for it before anything else. Spec unfindable after the search chain → the Spec axis reports "no spec available"; review proceeds.
...
- Findings use severity within each axis: **Critical** (broken behavior, security, spec violation — fix immediately), **Important** (fix before declaring done), **Minor** (fix if cheap, else note).
- Report persisted to `docs/审查/YYYY-MM-DD-<审查主题>.md` and registered in 文档导航.

## Constraints and Prohibitions

- Pin the fixed point first: `git diff <fixed-point>...HEAD` (three-dot, merge-base) and `git log <fixed-point>..HEAD --oneline`. Confirm the ref resolves and the diff is non-empty BEFORE spawning any sub-agent — a bad ref or empty diff fails here, not inside the agents.
- Spec source order: commit-message issue refs (via `docs/agents/issue-tracker.md` if present; skip if missing) → user-passed path → `docs/设计/`, `docs/`, `specs/`, `.scratch/` matching the branch/feature → ask the user.
- Standards sources: `docs/规范.md` first, then `CODING_STANDARDS.md` / `CONTRIBUTING.md`; the smell baseline is pasted in full into the Standards sub-agent regardless (it has no other access).
- The two axes never merge, never re-rank against each other, never substitute for each other — a change can pass one and fail the other.
- Prohibited: implementing findings without the reception discipline (receiving-code-review); presenting a merged single report.
...
- Findings disputed → receiving-code-review owns verification and pushback.
...
- Log: the persisted report IS the record; disputes and pushbacks land in receiving-code-review's trail.
