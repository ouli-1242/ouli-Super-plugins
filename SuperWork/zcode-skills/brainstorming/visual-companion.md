...
**opencode (Windows):**
```powershell
# PowerShell variant of start-server.sh. Backgrounding is native here
# (Start-Process spawns a real Win32 process), so no --foreground needed.
.\scripts\start-server.ps1 -ProjectDir C:\path\to\project -Open
```

With Git Bash installed, the .sh variant works too: `bash scripts/start-server.sh --project-dir /path/to/project --open`
...
On Windows PowerShell: `.\scripts\stop-server.ps1 <session_dir>` (equivalent, native).
