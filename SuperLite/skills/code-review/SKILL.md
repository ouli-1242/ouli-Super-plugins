...
The issue tracker should have been provided to you — if `docs/agents/issue-tracker.md` is missing, note it and rely on commit-message issue references, the user-passed spec path, and specs under `docs/`, `specs/`, or `.scratch/`.
...
**If no sub-agent tool is available in this environment:** run both axes yourself, sequentially, keeping the two reports strictly separate (the separation is the point — don't let one axis's findings bleed into the other's report). Then continue with aggregation as below.
...
Before implementing any finding: verify it against the codebase first, and push back with technical reasoning when a finding is wrong. Never implement findings blindly.
