...
description: "Use when a bug resists a first fix attempt, or the user reports something broken, throwing, failing, or slow - \"debug\", \"帮我调试\", \"为什么报错\", \"这个 bug 怎么回事\", performance regressions. Builds a tight feedback loop BEFORE hypothesizing or fixing anything. NOT for trivial one-line errors with an obvious cause - fix those directly. NOT for new feature work - that is not a bug. NOT for just explaining what an error means when no fix is asked - answer that directly."
---
# Diagnosing Bugs
...
- Do NOT use: trivial one-line errors with an obvious cause (fix directly); new feature work (that is not a bug); explaining an error when no fix is asked (answer directly).
...
- The report is really a feature request → say so and stop; that is not a bug.
...
- Log: post-mortem (cause + prevention finding) into the commit/PR and `docs/日志/` when a project exists.
