...
- Use: writing or about to write production code — any feature or bug fix.
- Do NOT use: requirements not yet confirmed (ask the user first); documentation or analysis work (writing).
...
- Required: a task with a clear deliverable (feature description or bug with a clear fix path).
- Optional: project seam declarations (`docs/测试.md`).
- Missing behavior: requirements not confirmed → stop and ask the user before writing any code.
...
- Log: seam agreements recorded in the task/commit; violations to `docs/日志/` when a project exists.
