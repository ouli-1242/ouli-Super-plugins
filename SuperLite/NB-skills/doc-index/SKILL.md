...
- Use: `docs/文档导航.md` doesn't exist and an artifact needs a home; the user asks to set up or reorganize project docs (「建个文档导航」「整理项目文档结构」).
...
- Optionally a root `AGENTS.md` onboarding file pointing at the index and the disciplines.
...
- **Default layout**: `docs/` holds 决策记录.md, 技术依据.md, 规范.md, 运行.md, 测试.md at root, plus 调研/ 设计/ 计划/ 日志/ 审查/ 交接/ 实验/ (each with a declared purpose). Optional directories (归档/, 变更志.md, 追溯矩阵.md, 风险登记.md) exist only when declared in the index.
...
- Positive: writing finishes a durable plan → saved to `docs/计划/2026-09-19-双十一方案.md` → one row appended → done.
- Negative: a research summary left only in chat in a project with a docs spine — the artifact does not exist as far as the protocol is concerned.
