...
- 改动后跑 `powershell -File scripts/validate-multi.ps1 -AllTargets` 与 `powershell -File scripts/validate-nb.ps1`。
...
**已知限制**：同一 harness 内按任务切换模型（如 `/model`）时，skill 变体是静态安装产物，无法跟随模型动态切换——变体按 harness 的主力模型在 `agents.json` 的 `model_tiers` 里登记，换主力模型即重装对应变体。
