---
name: superoffice
description: "Use when any office-document task starts - drafting reports, memos, letters, slides, meeting notes; summarizing messy input; reviewing someone's material; filling forms; spreadsheet work; or any Word/Excel/PPT/PDF task. 中文信号：写报告、做PPT、改格式、整理会议记录、审材料"
---
# SuperOffice Router

Decide **which verb** the current task needs, then invoke that skill by name. One task → one verb skill at a time; the verbs are mutually exclusive, chained in sequence when a task spans several. When two seem to apply, the workflow position decides: input comes before drafting, drafting before revision, revision before review.
...
| 修改 | "润色", "改短一点", "换个语气", "排版乱了帮我修", "改一下措辞" | `doc-revise` |
| 审阅 | "看看这个方案行不行", "审一下合同", "给点意见", "帮我批注" | `doc-review` |
| 数据 | "汇总这几个表", "核对数据", "做个透视", "统计一下", "图表" | `data-report` |
| 文件手术 | "转成PDF/Word", "合并/拆分", "把扫描件转成表格", "提取这页的数据" | `file-ops` |
| 填表 | "填一下这个申报表", "把资料填进这个模板" | `form-fill` |
| 验证交付 | "文件做好了吗", "检查一下能不能交", deliverable ready to hand over | `verify-output` |
| 沉淀资产 | "记住我们单位的模板", "以后都按这个格式", "把这份样例存下来" | `doc-asset` |
| 配置环境 | "配置MCP", "装个office MCP", "让AI能操控WPS（首次装机）", first-time setup on a machine | `office-mcp-setup` |
| 驱动WPS | "用wps-cli", "操控WPS（已装好）", "公式不更新需要重算", "WPS导出PDF" | `wps-cli` |
...
None of the four disciplines has exemptions. Trivial one-line edits (fix a typo, rename a file) may skip routing entirely.
